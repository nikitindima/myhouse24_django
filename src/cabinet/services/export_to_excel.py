...
def WriteToExcel(weather_data, town=None):
    ...
    for idx, data in enumerate(weather_data):
        ...
        worksheet_s.write_formula(
            row_index, 5, "=AVERAGE({0}{1}:{0}{2})".format("C", 6, row_index + 1)
        )
        worksheet_s.write_formula(
            row_index, 6, "=AVERAGE({0}{1}:{0}{2})".format("D", 6, row_index + 1)
        )
        worksheet_s.write_formula(
            row_index, 7, "=AVERAGE({0}{1}:{0}{2})".format("E", 6, row_index + 1)
        )
